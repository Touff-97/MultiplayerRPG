...
func get_drag_data(_position: Vector2):
	...
	if gui.inventory_data[inv_slot]["Item"] != null:
		# Setting the slot's data if it has an item
		var data := {}
		data["origin_node"] = self
		data["origin_panel"] = "Inventory"
		data["origin_item_id"] = gui.inventory_data[inv_slot]["Item"]
		data["origin_equipment_slot"] = gui.item_data[str(gui.inventory_data[inv_slot]["Item"])]["EquipmentSlot"]
		data["origin_max_stack"] = gui.item_data[str(gui.inventory_data[inv_slot]["Item"])]["Stack"]
		data["origin_stack"] = gui.inventory_data[inv_slot]["Stack"]
...
func can_drop_data(_position: Vector2, data) -> bool:
	...
	if gui.inventory_data[target_inv_slot]["Item"] == null:
		...
		if Input.is_action_pressed("secondary"):
			...
			data["target_item_id"] = gui.inventory_data[target_inv_slot]["Item"]
			data["target_texture"] = texture
			data["target_stack"] = gui.inventory_data[target_inv_slot]["Stack"]
			data["target_max_stack"] = gui.item_data[str(gui.inventory_data[target_inv_slot]["Item"])]["Stack"]
			if data["origin_panel"] == "Equipment":
				var target_equipment_slot = gui.item_data[str(gui.inventory_data[target_inv_slot]["Item"])]["EquipmentSlot"]
...
func drop_data(_position: Vector2, data) -> void:
	...
	if data["origin_node"] == self:
		...
		split_popup_instance.rect_position = get_parent().get_global_transform_with_canvas().origin - Vector2(200, 0)
		...
		if data["target_item_id"] == data["origin_item_id"] and data["target_max_stack"] > 1 and (data["origin_stack"] + data["target_stack"]) <= data["target_max_stack"]:
			...
			gui.inventory_data[target_inventory_slot]["Stack"] = new_stack
		...
		# Elif statement with a difference in stack greater than the max stack = texture is the same, stack label = max stack
			...
			gui.inventory_data[target_inventory_slot]["Stack"] = data["target_max_stack"]
			get_node("../Stack").set_text(str(new_stack))
		else:
			gui.inventory_data[target_inventory_slot]["Item"] = data["origin_item_id"]
			texture = data["origin_texture"]
			gui.inventory_data[target_inventory_slot]["Stack"] = data["origin_stack"]
...
func SplitStack(split_amount: int, data: Dictionary) -> void:
	...
	gui.inventory_data[origin_slot]["Stack"] = data["origin_stack"] - split_amount
	gui.inventory_data[target_inv_slot]["Item"] = data["origin_item_id"]
	gui.inventory_data[target_inv_slot]["Stack"] = split_amount
	texture = data["origin_texture"]
